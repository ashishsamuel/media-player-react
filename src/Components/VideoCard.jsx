...
function VideoCard({video,updateDeleteStatus}) {
  ...
  return (
    <>
    { video &&
      <Card className='my-3' style={{height:'350px'}} onDragStart={(e)=>dragStart(e,video?.id)} draggable>
      <Card.Img className='w-100' height='250px' style={{cursor:'pointer'}} variant="top" src={video?.url} onClick={()=>openVideo(video)}/>
    ...
    <Modal
      ...
      >
        ...
        <Modal.Body>
          <iframe width={'100%'} height="200" src={`${video.embedlink}?autoplay=1&mute=1`} title={video.caption} frameborder="0" allowFullScreen></iframe>
        </Modal.Body>
